...
struct file
{
        unsigned int length; // total number of bytes of the file
...
void file_print(void *f)
{
	...
	printf("file %s of %u bytes.", fs->path, fs->length);
}
...
struct metadata_info
{
	char *name; // name of the file (for single file download) or directory (for download
		    // of multiple files)
	unsigned int piece_length; // number of bytes of each piece that constitutes the download
...
{
	unsigned char metadata_info_downloaded; // 0 if metadata_info has not been downloaded
						// 1 if it has.
	unsigned int curr_piece_index_downloading; // 0 starting index of the current piece
	...
	unsigned int piece_length; // number of bytes each pieice is
	unsigned int subpiece_length; // number of bytes each subpiece of a piece is
	...
	int optmisitic_unhchoked_peers[5]; // array of peer incdices that are being currently
...
struct peer_metadata
{
	...
	int request_pending; // -1 if no request for a subpiece is pending	
			     // is the 0 starting index of the subpiece that was requested
			     // and that is pending
	int subpiece_pipeline[5]; // int value of array is -1 to indicate an abscent subpiece 
	...
	int data_downloaded[6]; // data_downloaded[0] is the amount of data download
